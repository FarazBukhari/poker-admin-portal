...
const TournamentListHome = () => {
    ...
    const { total_pages } = useSelector((state) => state.tournamentList);
    const [pageNum, setPageNum] = useState(1);
    const [rowsPerPage, setRowPerPage] = useState(10);
    const [applyFilter, setApplyFiler] = useState(true);
    const [searchString, setSearchString] = useState();
    useEffect(() => {
        dispatch(getDataCreateSuccess(pageNum, rowsPerPage, 'all'));
                ...
                >
                    ...
                    <Grid item xs={12} sx={{ p: 3 }}>
                        <Grid container justifyContent="space-between" spacing={gridSpacing}>
                            <Grid item>
                                <Pagination count={Math.ceil(total_pages / count)} page={pageNum} onChange={handleChange} color="primary" />
                            </Grid>
                            <Grid item>
                                <Button
                                    size="large"
