...
import { getTournamentTypeSuccess, getDetailIdSuccess } from 'store/slices/tournamentList';
...
const TournamentList = () => {
    ...
    const tournaments = useSelector((state) => state.tournamentList.data.tournaments);
                    ...
                    {tournaments &&
                        tournaments.map((item, index) => (
                            <TableRow hover key={index}>
                                ...
                                <TableCell sx={{ paddingLeft: '20px', width: '300px' }}>{item.name || 'No name set'}</TableCell>
                                <TableCell>Texas Holdem</TableCell>
                                <TableCell sx={{ paddingLeft: '30px' }}>{item.starting_chips || 0}</TableCell>
                                <TableCell sx={{ paddingLeft: '30px' }}>{item.total_entries || 0}</TableCell>
                                <TableCell>
                                ...
                                <TableCell sx={{ paddingLeft: '30px' }}>
                                    {new Date().getDate() <= new Date(item.registration_start_time).getDate() && (
                                        <Chip label="Open" size="small" chipcolor="info" />
                                    )}
                                    {new Date().getDate() > new Date(item.registration_start_time).getDate() &&
                                        new Date().getDate() <= new Date(item.registration_end_time).getDate() && (
                                ...
                                <TableCell sx={{ pr: 3 }}>
                                    <IconButton
                                        ...
                                        size="small"
                                        aria-label="edit"
                                        onClick={() => handleEdit(item.tournament_id)}
                                        sx={{ marginRight: '10px' }}
                                    >
                                    ...
                                    <IconButton
                                        ...
                                        size="small"
                                        aria-label="delet"
                                        onClick={() => handleDelete(item.tournament_id)}
                                        sx={{ marginLeft: '10px' }}
                                    >
